...
ExUnit.after_suite(fn _ ->
  ...
  Enum.map(File.ls!("#{storage_path}"), fn study_dir ->
    if File.dir?("#{storage_path}/#{study_dir}") do
      Enum.map(File.ls!("#{storage_path}/#{study_dir}"), fn part_dir ->
        Enum.map(File.ls!("#{storage_path}/#{study_dir}/#{part_dir}"), fn file ->
          File.rm!("#{storage_path}/#{study_dir}/#{part_dir}/#{file}")
        end)

        File.rmdir!("#{storage_path}/#{study_dir}/#{part_dir}")
      end)
